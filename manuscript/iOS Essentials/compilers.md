...
What we can do to solve the size issue in CLR(1) parsing table is that we can merge the states which have the same LR(0) items. For example if I3 and I6 has the same LR(0) items but only the lookaheads are same then merge them into one state call it as I3,6. This is called as the LALR(1) parsing table. See the number of states in LR(0) and SLR(1) is the same. In CLR(1) the number of states may increase because we may have different lookaheads, but by combining the states that have the same LR(0) items we are making the lookaheads irrelevant. Thus the number of states in LALR(1) = LR(0) = SLR(1) = x, in CLR(1) we may have more than or equal to x states. For getting the LALR(1) parsing table in all the shift and goto entries and the row names replace with the changes state name, i.e where you have 3 or 6 now replace with 3,6 but don't touch the reduce entries. Then merge the rows that are the same states.